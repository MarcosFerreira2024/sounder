...
class GameModesRepository implements IGameModesRepository {
    ...
    async getGameModes( search?: gameModesQueryFilters,page?: number, limit?: number): Promise<GameMode[]> {
        ...
        return await prisma.gameMode.findMany({
            where: {
                  ...
                  ...(search?.description &&  { description: { contains: search.description } }),
            },
            take: limit && limit,
    ...
    async update(id: string, data: { name?: string; description?: string; }): Promise<GameMode> {
