...
class CreateDailyGame {
    ...
    constructor(
        ...
        @inject("DailyGameRepository") private dailyGameRepository: IDailyGameRepository,
        @inject("GameHistoryRepository") private gameHistoryRepository: IGameHistoryRepository,
        @inject("MusicRepository") private musicRepository: IMusicRepository,
        @inject("ImageProcessingService") private imageProcessingService: IImageProcessingService
    ) {}
    ...
    async execute() {
        ...
        const alreadyCreatedMusicsNames = alreadyCreatedHistory.map(music => music.musicName)

        let randomMusic = await this.musicRepository.getRandomMusic(alreadyCreatedMusicsNames)
        let tries = 0

        while(randomMusic === null) {
            randomMusic = await this.musicRepository.getRandomMusic(alreadyCreatedMusicsNames)
            tries++
        ...
        const album = await this.albumRepository.getAlbumById(randomMusic.albumId as string)

        if(!album) throw new Error("Album not found")

        const albumCoverDirectory = path.dirname(album.cover);

        const blurPaths = await this.imageProcessingService.ensureBlurImages(album.cover, albumCoverDirectory);
        ...
        const game = await this.dailyGameRepository.createDailyGame({
            ...
            originalImage: randomMusic.album!.cover,
        });
