...
class ArtistController {
    ...
    async assignUserAsArtist(req: Request, res: Response) {
        ...
        try {
            ...
            await container.resolve(AssignUserAsArtist).execute(req.user!,req.params.userId as string)
    ...
    async findArtists(req: Request, res: Response) {
        ...
        try {
            ...
            const artists = await container.resolve(FindArtists).execute(search,page,limit)


            return res.status(200).json({data:artists, message:"Artists fetched successfully"})
        }
