...
class GetUserPlaylists {
    ...
    async execute(userId?: string, user?: AppUser, page?: number, limit?: number):Promise<{items:Partial<Playlist>[], page:number, totalItems:number}> {
    if (!user && !userId) throw new Error("User id or Authenticated user is required");
    ...
    let playlists = await this.playlistRepository.getPlaylistByUserId(targetUserId!, page, limit);
